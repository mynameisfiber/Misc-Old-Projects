...
def update_mhi( img, dst, diff_threshold ):
    ...
    for i in range(-1, seq.total):
        ...
        if magnitude == 100:
          ...
          #cvCircle( dst, center, cvRound(radius), color, 3, CV_AA, 0 );
          cvRectangle(dst, cvPoint(comp_rect.x, comp_rect.y), cvPoint(comp_rect.x+comp_rect.width, comp_rect.y+comp_rect.height), 
                      color, 3, CV_AA, 0)
          cvLine( dst, center, cvPoint( cvRound( center.x + comp_rect.width / 2 *cos(angle*CV_PI/180)),
                  cvRound( center.y - comp_rect.height / 2 *sin(angle*CV_PI/180))), color, 3, CV_AA, 0 );
