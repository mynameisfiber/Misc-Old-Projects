...
implicit none
  ...
  REAL, DIMENSION(4,n,n,n) :: u
  REAL :: CFL=0.65, dt=0.0, maxv=0.0
  REAL :: start, finish
  INTEGER :: i,j,k
  ...
  PRINT*,"Solving"
  call cpu_time(start)
  CALL step(u,n,CFL,dt,maxv)
  call cpu_time(finish)

  PRINT*,"Seconds to solve: ",(finish-start)
...
CONTAINS
  ...
  subroutine step(u, n, CFL, dt, maxv)
    ...
    DO k=1,n
      maxv = max(maxval(abs(u(2,:,:,k)/u(1,:,:,k))),maxval(abs(u(3,:,:,k)/u(1,:,:,k))),maxval(abs(u(4,:,:,k)/u(1,:,:,k))))
    END DO
    !$OMP END PARALLEL DO

    !Calculate the timestep based on the courant condition
    if (dt .eq. 0.0) dt = CFL / (csound+maxv) 
    !perform strang splitting using the hydro only rTVD algorithm
    !given by http://arxiv.org/abs/astro-ph/0305088
  ...
  RECURSIVE SUBROUTINE tvdeuler(u,n,dt,maxv) 
    ...
    INTEGER :: n, k
    REAL :: dt, maxv, c
    REAL, DIMENSION(4,n) :: u, uhalf, flux 
    REAL, DIMENSION(4,n) :: fluxR, fluxL
    REAL, DIMENSION(4,n) :: fwr, fwl
...
!      end if
  ...
  function tvdflux(f,n) result(flux) 
    ...
    INTEGER :: n
    REAL, DIMENSION(4,n) :: f
    REAL, DIMENSION(4,n) :: flux, fr, fl, r, psi
  ...
  function getflux(u,n) result(flux)!derive physical flux from state --EULER
    ...
    !f2py depend(u) n
    INTEGER :: n
    REAL, DIMENSION(4,n) :: u
    REAL, DIMENSION(4,n) :: flux
    REAL, DIMENSION(n) :: v 
